 ...
 * Incluye funcionalidades comunes de CRUD y validación
 * 
...
abstract class Model {
    ...
    /**
     * Obtener nombre de tabla automáticamente
     */
    protected function getTableName() {
        $className = strtolower(get_class($this));
        return $className . 's'; // Pluralización simple
    }
    ...
    public function create($data) {
        try {
            ...
            if (!empty($validationErrors)) {
                throw new Exception('Datos inválidos: ' . implode(', ', $validationErrors));
            }
    ...
    protected function transformResult($result) {
        ...
        foreach ($this->casts as $key => $type) {
            if (isset($result[$key])) {
                $result[$key] = $this->castAttribute($result[$key], $type);
            }
        }
    ...
    /**
     * Métodos mágicos para propiedades dinámicas
     */
